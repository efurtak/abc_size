  ...
  describe '#call' do
    ...
    context 'when invoking with path' do
      ...
      context 'when file not exists' do
        let(:path) { 'lib/not_existing_file.rb' }

        it 'exit on Errno::ENOENT' do
          expect { with_path }.to raise_error(SystemExit)
        end
      end

      context 'when file not exists because path is a directory' do
        let(:path) { 'lib/' }

        it 'exit on Errno::EISDIR' do
          expect { with_path }.to raise_error(SystemExit)
        end
      end

      context 'when file exists' do
        let(:path) { 'lib/abc_size.rb' }

        it 'not raises error and not exit' do
          expect { with_path }.not_to raise_error
        end
      end

      context 'when file exists but is empty' do
        let(:path) { 'spec/fixtures/files/empty_file.rb' }
